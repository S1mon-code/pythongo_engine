"""
================================================================================
  AL_Long_1H_V15 — EMA + RSI + ADX Triple Confirmation (电解铝 H1做多)
================================================================================

  QBase_v3 策略: i_long_v15_ema_rsi_adx_triple
  信号: EMA趋势(close>EMA60) + RSI动量[45,75] + ADX强度(>22)+PDI>MDI
        三重门控全部通过才出信号，加权复合: EMA 30% + RSI 40% + ADX 30% → [0, 1]
...
# ══════════════════════════════════════════════════════════════════════════════
#  SIGNAL — EMA trend + RSI momentum + ADX strength (triple confirmation)
# ══════════════════════════════════════════════════════════════════════════════

def generate_signal(closes, highs, lows, bar_idx):
    """三重门控 + 加权复合信号 → [0, 1]. Long only.

    Gate 1: close > EMA(60)
    Gate 2: RSI(14) in [45, 75]
    Gate 3: ADX(14) > 22 and PDI > MDI
    Composite: EMA_comp*0.3 + RSI_comp*0.4 + ADX_comp*0.3
    """
    if bar_idx < WARMUP:
        return 0.0
    ...
    if any(np.isnan(x) for x in (ema_val, r, a, pdi, mdi)):
        return 0.0
    ...
    ema_component = min((close - ema_val) / ema_val * 50.0, 1.0)
    rsi_component = (r - RSI_FLOOR) / (RSI_CEILING - RSI_FLOOR)
    adx_component = min((a - ADX_THRESHOLD) / 20.0, 1.0)

    signal = ema_component * 0.3 + rsi_component * 0.4 + adx_component * 0.3
...
class AL_Long_1H_V15_EMA_RSI_ADX_Triple(BaseStrategy):
    """电解铝 H1做多 — EMA + RSI + ADX Triple Confirmation"""
    ...
    def _on_bar(self, kline: KLineData):
        ...
        self.output(
            f"[IND] EMA{EMA_PERIOD}={e_val:.1f} RSI{RSI_PERIOD}={r_val:.1f} "
