"""
================================================================================
  AL_Long_4H_V20 — Donchian Breakout + EMA Filter + ATR Vol Regime (电解铝4H做多)
================================================================================

  信号: Donchian突破 + EMA趋势过滤 + ATR波动率regime衰减 → [0, 1]
        三重门控: 价格>EMA + 价格>Donchian中轨 + ATR不极端
...
STRATEGY_NAME = "AL_Long_4H_V20"

# 策略指标参数 (QBase_v3 默认值)
DC_PERIOD = 40
EMA_PERIOD = 80
ATR_PERIOD = 14
ATR_MULT_CEILING = 2.5
WARMUP = 100
...
# ══════════════════════════════════════════════════════════════════════════════
#  SIGNAL — Donchian breakout + EMA filter + ATR vol regime
# ══════════════════════════════════════════════════════════════════════════════
...
def generate_signal(closes, highs, lows, bar_idx):
    ...
    vol_factor = 1.0 - min((a / atr_med - 1.0) / (ATR_MULT_CEILING - 1.0), 0.5)
...
class AL_Long_4H_V20_Donchian_EMA_ATR(BaseStrategy):
    """电解铝 H4做多 — Donchian Breakout + EMA Filter + ATR Vol Regime"""
    ...
    def _on_bar(self, kline: KLineData):
        ...
        self.output(
            f"[IND] EMA{EMA_PERIOD}={e_val:.1f} ATR={a_val:.2f} "
        ...
        if self._pending is None and net_pos > 0:
            ...
            if chandelier_long(highs, closes, ch_atr, bar_idx):
                self._pending = "CLOSE"
                self._pending_reason = "Chandelier Exit"
