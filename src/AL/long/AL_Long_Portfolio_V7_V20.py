"""
================================================================================
  AL_Long_Portfolio_V7_V20 — V7+V20合并信号 (电解铝 H4做多)
================================================================================

  QBase_v3 Portfolio: AL多头
  信号: V7(EMA Trend + RSI Filter) + V20(Donchian Breakout + EMA + ATR Regime)
        合并 = (v7 + v20) / 2 → [0, 1]
  ...
  架构:
  - 单KLineGenerator (H4) — V7和V20共用同一周期
  - next-bar执行规则
...
STRATEGY_NAME = "AL_Long_Portfolio_V7_V20"
...
# ── V20 参数 (Donchian Breakout + EMA Filter + ATR Vol Regime) ──
V20_DC_PERIOD = 40
V20_EMA_PERIOD = 80
V20_ATR_PERIOD = 14
...
# ══════════════════════════════════════════════════════════════════════════════
#  SIGNAL V7 — EMA trend + RSI bell-curve filter
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
#  SIGNAL V20 — Donchian breakout + EMA filter + ATR vol regime
# ══════════════════════════════════════════════════════════════════════════════
...
class AL_Long_Portfolio_V7_V20(BaseStrategy):
    """电解铝 H4做多 — V7(EMA+RSI) + V20(Donchian+EMA+ATR) 合并信号"""
    ...
    def on_start(self):
        ...
        if saved:
            ...
            self._signal_v7 = saved.get("signal_v7", 0.0)
            self._signal_v20 = saved.get("signal_v20", 0.0)
    ...
    def _on_bar(self, kline: KLineData):
        ...
        self.output(
            f"[IND] V7: EMA{V7_EMA_PERIOD}={e7:.1f} RSI{V7_RSI_PERIOD}={r7:.1f} close={close:.1f}"
        )
        ...
        self.output(
            f"[IND] V20: EMA{V20_EMA_PERIOD}={e20:.1f} ATR={a20:.2f} "
            f"DC_upper={u20:.1f} DC_mid={m20:.1f}"
        )

        # ── 合并信号 ──
        self._signal_v7 = generate_signal_v7(closes, bar_idx)
        self._signal_v20 = generate_signal_v20(closes, highs, lows, bar_idx)
        combined = (self._signal_v7 + self._signal_v20) / 2.0
        forecast = min(FORECAST_CAP, max(0.0, combined * FORECAST_SCALAR))

        self.state_map.signal_v7 = round(self._signal_v7, 3)
        self.state_map.signal_v20 = round(self._signal_v20, 3)
        self.state_map.combined = round(combined, 3)
        self.state_map.forecast = round(forecast, 1)
        self.output(
            f"[IND] V7={self._signal_v7:.3f} V20={self._signal_v20:.3f} combined={combined:.3f} forecast={forecast:.1f} close={close:.1f}"
        )
        self.output(
            f"[SIGNAL] V7={self._signal_v7:.4f} V20={self._signal_v20:.4f} "
        ...
        if self._pending is None and not self._twap.is_active and target != net_pos:
            ...
            self._pending_reason = (
                f"V7={self._signal_v7:.2f} V20={self._signal_v20:.2f} "
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  TWAP提交 (LONG: open/add=buy, close/reduce=sell)
    # ══════════════════════════════════════════════════════════════════════
    ...
    def _submit_twap(self, kline: KLineData, action: str):
        ...
        feishu("info", p.instrument_id,
               f"**TWAP启动** {action}\n"
               f"目标: {vol}手 {direction}\n"
               f"窗口: 第2-11分钟\n"
               f"逻辑: {self._pending_reason}")

    # ══════════════════════════════════════════════════════════════════════
    #  立即执行 (仅止损信号, LONG: open=buy, close=sell)
    # ══════════════════════════════════════════════════════════════════════
    ...
    def _save(self):
        state = {
            ...
            "signal_v7": self._signal_v7,
            "signal_v20": self._signal_v20,
        ...
        if self._twap.is_active:
            ...
            if not self._twap.is_active:
                feishu("info", self.params_map.instrument_id,
                       ...
                       f"成交: {self._twap.progress} VWAP={self._twap.vwap:.1f}")
