"""
================================================================================
  IH_Long_Portfolio_V7_V20 — V7+V20 合并信号 (上证50 H4做多)
================================================================================

  Portfolio组合:
    V7  — EMA Trend + RSI Filter (EMA趋势 + RSI甜蜜区钟形曲线)
    V20 — Donchian Breakout + EMA Filter + ATR Vol Regime (三重门控)
  信号: combined = (v7 + v20) / 2  → [0, 1]
...
STRATEGY_NAME = "IH_Long_Portfolio_V7_V20"

# ── V7: EMA Trend + RSI Filter ──
V7_EMA_PERIOD = 60
V7_RSI_PERIOD = 14
V7_RSI_FLOOR = 40.0
V7_RSI_CEILING = 70.0

# ── V20: Donchian Breakout + EMA + ATR Vol Regime ──
V20_DC_PERIOD = 40
V20_EMA_PERIOD = 80
V20_ATR_PERIOD = 14
...
FORECAST_CAP = 20.0
ANNUAL_FACTOR = 252 * 1               # H4: IH无夜盘, ~1 bar/day
...
# ══════════════════════════════════════════════════════════════════════════════
#  INDICATORS (inline, 纯numpy, 从两路QBase策略移植)
# ══════════════════════════════════════════════════════════════════════════════
...
class IH_Long_Portfolio_V7_V20(BaseStrategy):
    """上证50 H4做多 — V7(EMA+RSI) + V20(Donchian+EMA+ATR) Portfolio合并"""

    def __init__(self):
        super().__init__()
        self.params_map = Params()
        self.state_map = State()
        self.kline_generator = None          # 单一 H4 KLineGenerator，两路共享
    ...
    def on_start(self):
        ...
        feishu("start", p.instrument_id,
               ...
               f"组合: V7(EMA+RSI) + V20(Donchian+EMA+ATR)")
    ...
    def _on_bar(self, kline: KLineData):
        ...
        rsi_arr = _rsi(closes, V7_RSI_PERIOD)
        e7_val = ema_v7[bar_idx] if not np.isnan(ema_v7[bar_idx]) else 0.0
        ...
        atr_arr = _atr(highs, lows, closes, V20_ATR_PERIOD)
        dc_upper, dc_lower, dc_mid = _donchian(highs, lows, V20_DC_PERIOD)
        e20_val = ema_v20[bar_idx] if not np.isnan(ema_v20[bar_idx]) else 0.0
        a_val = atr_arr[bar_idx] if not np.isnan(atr_arr[bar_idx]) else 0.0
        u_val = dc_upper[bar_idx] if not np.isnan(dc_upper[bar_idx]) else 0.0
        m_val = dc_mid[bar_idx] if not np.isnan(dc_mid[bar_idx]) else 0.0

        self.output(
            f"[IND] V7: EMA{V7_EMA_PERIOD}={e7_val:.1f} RSI{V7_RSI_PERIOD}={r_val:.1f} | "
            f"V20: EMA{V20_EMA_PERIOD}={e20_val:.1f} ATR={a_val:.2f} "
            f"DC_upper={u_val:.1f} DC_mid={m_val:.1f} | close={close:.1f}"
        )
        ...
        # ── 持仓追踪 (多头: peak=最高价, net_pos正数, 无abs()) ──
        if net_pos == 0:
            self.avg_price = 0.0
            self.peak_price = 0.0
        ...
        # ── 止损检查 (多头: RiskManager直接处理, net_pos正数) ──
        if net_pos > 0:
