"""
================================================================================
  Template_Long — 做多策略模板 (PythonGO Engine)
================================================================================

  使用方法:
    1. 复制本文件到 {品种}/long/ 目录
    2. 重命名文件: {品种}_Long_{频率}_{版本}_{指标简称}.py
...
STRATEGY_NAME = "XX_Long_H1_V1"          # ← CUSTOMIZE: 策略唯一名
...
def generate_signal(closes, bar_idx):
    ...
    # ← CUSTOMIZE: 替换为实际信号逻辑, 返回 [0, 1]
    if close <= e:
        return 0.0
    if r < 40 or r > 70:
        return 0.0
    return float(np.clip((r - 40) / 15.0, 0.0, 1.0))
...
class Params(BaseParams):
    ...
    instrument_id: str = Field(default="al2605", title="合约代码")  # ← CUSTOMIZE
...
class Template_Long(BaseStrategy):                # ← CUSTOMIZE: 类名 = 文件名
    """做多策略模板"""                              # ← CUSTOMIZE: 描述
