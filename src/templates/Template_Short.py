"""
================================================================================
  Template_Short — 做空策略模板 (PythonGO Engine)
================================================================================

  使用方法:
    1. 复制本文件到 {品种}/short/ 目录
    2. 重命名文件: {品种}_Short_{频率}_{版本}_{指标简称}.py
    3. 类名改成和文件名一致 (PythonGO硬性要求)
    4. 搜索 "← CUSTOMIZE" 修改所有自定义项
    5. 实现 INDICATORS 和 SIGNAL 部分的指标/信号逻辑
    6. 部署: modules/ → pyStrategy/modules/, 本文件 → self_strategy/

  与 Template_Long 的关键区别:
    - 信号范围 [-1, 0] (负值表示做空强度)
    - open=sell, close=buy (方向反转)
    - peak_price → trough_price (追踪最低价)
    - 止损: close >= avg*(1+pct) / close >= trough*(1+pct) (反向)
    - hard/trail止损内联处理, RiskManager只管equity/circuit/daily
    - net_pos显示负值, target_lots显示负值

  模板版本: 2026-04-16
  特性: Same-bar执行 / TWAP分批 / Chandelier Exit / 4层止损 / 飞书通知
...
STRATEGY_NAME = "XX_Short_H1_V1"          # ← CUSTOMIZE: 策略唯一名

# 策略指标参数                             # ← CUSTOMIZE: 替换为实际指标参数
EMA_PERIOD = 60
RSI_PERIOD = 14
WARMUP = 100                              # ← CUSTOMIZE: >= 最大指标窗口

# Chandelier Exit (Short)
CHANDELIER_PERIOD = 22
CHANDELIER_MULT = 2.5                     # ← CUSTOMIZE: 通常 2.0-3.0

# Vol Targeting
FORECAST_SCALAR = 10.0
FORECAST_CAP = 20.0
ANNUAL_FACTOR = 252 * 8                   # ← CUSTOMIZE: H1~8, H4~3, Daily~1 (视夜盘)
...
# ══════════════════════════════════════════════════════════════════════════════
#  INDICATORS (inline, 纯numpy)                                  ← CUSTOMIZE
# ══════════════════════════════════════════════════════════════════════════════
...
def generate_signal(closes, bar_idx):
    ...
    # ← CUSTOMIZE: 替换为实际信号逻辑, 返回 [-1, 0]
    if close >= e:
        return 0.0
    if r > 60 or r < 30:
        return 0.0
    return float(np.clip(-(60 - r) / 15.0, -1.0, 0.0))
...
class Params(BaseParams):
    exchange: str = Field(default="SHFE", title="交易所代码")       # ← CUSTOMIZE
    instrument_id: str = Field(default="cu2605", title="合约代码")  # ← CUSTOMIZE
    kline_style: str = Field(default="H1", title="K线周期")        # ← CUSTOMIZE
    max_lots: int = Field(default=2, title="最大持仓")             # ← CUSTOMIZE
    capital: float = Field(default=1_000_000, title="配置资金")     # ← CUSTOMIZE
...
class State(BaseState):
    ...
    trough_price: float = Field(default=0.0, title="谷价(空)")     # ← 空头用trough
...
class Template_Short(BaseStrategy):               # ← CUSTOMIZE: 类名 = 文件名
    """做空策略模板"""                              # ← CUSTOMIZE: 描述
    ...
    def __init__(self):
        ...
        # 持仓状态
        self.avg_price = 0.0
        self.trough_price = 0.0                   # ← 空头: 追踪最低价 (多头用 peak_price)
        self._pending = None
        ...
        self._perf = None
        self._multiplier = 5                      # ← CUSTOMIZE: 合约乘数默认值
    ...
    def _on_bar(self, kline: KLineData):
        ...
        # ── 信号计算 ──                            ← CUSTOMIZE: 调整 generate_signal 参数
        raw = generate_signal(closes, bar_idx)
        forecast = min(FORECAST_CAP, max(0.0, abs(raw) * FORECAST_SCALAR))  # ← abs() for short
        ...
        # ── 止损检查 (空头: hard/trail 内联, equity/circuit 走 RiskManager) ──
        if net_pos > 0:
            # Hard stop (空头): 价格上涨超过均价+pct%
            ...
            # Trail stop (空头): 价格从最低点反弹超过pct%
            if self._pending is None and self.trough_price > 0 and close >= self.trough_price * (1 + p.trailing_pct / 100):
            ...
            # Equity/Portfolio stops (hard/trail=999 跳过, 只查 equity/circuit/daily)
    ...
    def _submit_twap(self, kline: KLineData, action: str):
        ...
        if direction == "sell":                   # ← 空头: 开仓方向检查保证金
            acct = self._get_account()
            price = kline.close
    ...
    def on_trade(self, trade: TradeData, log=True):
        ...
        actual = abs(pos.net_position) if pos else 0
        if direction == "sell" and actual > 0:    # ← 空头: 卖=开/加仓
        ...
        elif direction == "buy" and actual <= 0:  # ← 空头: 买=全平
            self.avg_price = 0.0
            self.trough_price = 0.0
