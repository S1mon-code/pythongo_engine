"""
================================================================================
  CU_Short_1M_V26_TEST — OI Flow + MACD 做空测试版 (铜 M1)
================================================================================

  ⚠️ 仅用于模拟盘测试！M1级别信号频繁。
  信号逻辑与V26完全一致，仅时间框架改为1分钟。
...
STRATEGY_NAME = "CU_Short_1M_V26_TEST"
...
class Params(BaseParams):
    ...
    capital: float = Field(default=10_000_000, title="配置资金")
...
class CU_Short_1M_V26_TEST(BaseStrategy):
    """⚠️ 测试版 — 铜M1做空 V26 OI Flow + MACD"""
    ...
    def _on_bar(self, kline: KLineData):
        ...
        self.output(f"[DEBUG] bar_idx={bar_idx} closes={len(closes)} oi={len(oi)}")

        if bar_idx < WARMUP:
            self._push_widget(kline, signal_price)
            return

        # OI对齐: push_history_data可能多推几根bar，OI从callback收集会少几根
        if len(oi) < WARMUP:
            self.output(f"[SKIP] oi不足: oi={len(oi)} < warmup={WARMUP}")
            self._push_widget(kline, signal_price)
            return
        ...
        flow, _ = _oi_flow(closes, oi, volumes, OI_PERIOD)
        macd_line, macd_sig, _ = _macd(closes, MACD_FAST, MACD_SLOW, MACD_SIGNAL_PERIOD)
        of_val = flow[bar_idx] if not np.isnan(flow[bar_idx]) else -999
        ml_val = macd_line[bar_idx] if not np.isnan(macd_line[bar_idx]) else -999
        ms_val = macd_sig[bar_idx] if not np.isnan(macd_sig[bar_idx]) else -999
        self.output(f"[IND] OI_Flow={of_val:.4f} MACD={ml_val:.1f} Signal={ms_val:.1f} close={closes[bar_idx]:.1f}")
    ...
    def _exec_close(self, kline: KLineData, actual: int, action: str) -> float:
        labels = {"CLOSE": "信号平仓", "HARD_STOP": "硬止损", "TRAIL_STOP": "移动止损",
                  "EQUITY_STOP": "权益止损", "CIRCUIT": "熔断", "DAILY_STOP": "单日止损", "FLATTEN": "盘前清仓"}
