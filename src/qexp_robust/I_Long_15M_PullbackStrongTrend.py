"""
================================================================================
  I_Long_15M_PullbackStrongTrend — QExp robust 策略移植
================================================================================

  原始: QExp overlay/signals/pullback_strong_trend.py
        + config/symbols/i_pullback_strong_trend_15min.yaml
  Audit: 2026-04-27 pullback-family-v2, 8y Sharpe +0.374, ex-best Δ -0.15 (严格 ROBUST)

  信号 (binary fires, 4 个条件全满足):
    - trend z_60 > +1.0 (强 uptrend, 比 v2 +0.5 更严)
    - 20-bar rolling max 在最近 8 bar 内 (最近创新高)
    - (rolling_max - cur_close) / atr >= 1.5 (深度回撤至少 1.5 ATR)
    - cooldown >= 5 bars
...
from modules.qexp_signals import PullbackStrongTrendSignal
...
STRATEGY_NAME = "I_Long_15M_PullbackStrongTrend"
...
class Params(BaseParams):
    ...
    kline_style: str = Field(default="M15", title="K线周期")
...
class I_Long_15M_PullbackStrongTrend(BaseStrategy):
    """I 15min Pullback in Strong Trend — QExp robust strategy."""
    ...
    def __init__(self):
        ...
        # 信号实例 (保留 cooldown 状态)
        self._signal = PullbackStrongTrendSignal()
