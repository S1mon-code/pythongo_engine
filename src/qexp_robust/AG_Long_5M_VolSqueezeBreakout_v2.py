"""
================================================================================
  AG_Long_5M_VolSqueezeBreakout_v2 — QExp robust 策略移植
================================================================================

  原始: QExp overlay/signals/vol_squeeze_breakout_long_v2.py
        + config/symbols/ag_vol_squeeze_breakout_long_v2.yaml
  Audit: 2026-04-27 vol-stratified, 8y Sharpe +0.470, ex-best Δ -0.14 (严格 ROBUST)

  信号 (binary fires, 5 个条件全满足):
    - trend z_60 > +0.5 (uptrend filter)
    - cur_ATR / mean(60 hist ATR) <= 0.8 (squeeze)
    - close > 20-bar high (excl current) (breakout)
    - body / range >= 0.5 (强收盘)
    - cooldown >= 5 bars
...
from modules.qexp_signals import VolSqueezeBreakoutLongV2Signal
...
STRATEGY_NAME = "AG_Long_5M_VolSqueezeBreakout_v2"
...
class AG_Long_5M_VolSqueezeBreakout_v2(BaseStrategy):
    """AG 5min Vol Squeeze Breakout v2 — QExp robust strategy."""
    ...
    def __init__(self):
        ...
        # 信号实例 (保留 cooldown 状态)
        self._signal = VolSqueezeBreakoutLongV2Signal()
