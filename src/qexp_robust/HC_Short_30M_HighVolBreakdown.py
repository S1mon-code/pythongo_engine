"""
================================================================================
  HC_Short_30M_HighVolBreakdown — QExp robust 策略移植
================================================================================

  原始: QExp overlay/signals/high_vol_breakdown_short.py
        + config/symbols/hc_high_vol_breakdown_short_30min.yaml
  Audit: 2026-04-27 vol-stratified-short, 8y Sharpe +0.544, ex-best Δ -0.13 (严格 ROBUST)

  信号 (binary fires, 4 个条件全满足):
    - cur_ATR / mean(60 hist ATR) >= 1.3 (vol expansion, regime change 信号)
    - close < 20-bar low (excl current) (breakdown)
    - close < open AND body / range >= 0.5 (强阴线, 不是 wick)
    - cooldown >= 5 bars

  入场 (SHORT): signal fires → 直接开 3 手空 (max_lots=capacity, 不走 Carver)
  出场 (任一即平):
...
from modules.qexp_signals import HighVolBreakdownShortSignal
...
STRATEGY_NAME = "HC_Short_30M_HighVolBreakdown"
...
class Params(BaseParams):
    ...
    kline_style: str = Field(default="M30", title="K线周期")
...
class HC_Short_30M_HighVolBreakdown(BaseStrategy):
    """HC 30min High Vol Breakdown SHORT — QExp robust strategy."""
    ...
    def __init__(self):
        ...
        # 信号实例 (保留 cooldown 状态)
        self._signal = HighVolBreakdownShortSignal()
        ...
        self._multiplier = 10  # HC default
    ...
    def on_tick(self, tick: TickData):
        ...
        # 主图/副图: 更新止盈/止损线 (SHORT — hard_stop 在上方, profit_target 在下方)
    ...
    def _on_tick_stops(self, tick: TickData):
        """Tick 级硬止损 + 止盈 — 只基于 own_pos. SHORT version."""
        if not self.trading:
            return
        ...
        # 1. Hard stop (SHORT): price ≥ entry × (1 + hard_stop_pct%)
        ...
        # 2. Profit target (SHORT): price ≤ entry − atr_mult × entry_ATR
        if self._entry_atr > 0:
    ...
    def _exec_stop_at_tick(self, price: float, action: str, reason: str) -> None:
        """Tick 触发的止损 / 止盈立即执行 (SHORT — 买入平空, 穿盘口 urgent)."""
        p = self.params_map
        if self._guard is not None and not self._guard.should_trade():
            return
        vol = self._own_pos
        ...
        buy_price = self._pricer.aggressive(price, "buy", urgency="urgent")
        self._log(f"[EXEC_STOP] {action} auto_close buy {vol}手 @ {buy_price} (urgent)")
    ...
    def _exec_open(self, signal_price: float, atr: float) -> None:
        ...
        sell_price = self._pricer.aggressive(signal_price, "sell", urgency="passive")
        self._log(
            f"[EXEC_OPEN] send_order sell {vol}手 @ {sell_price} (passive) "
            f"signal_price={signal_price:.2f} atr={atr:.2f}"
        )
        oid = self.send_order(
            exchange=p.exchange, instrument_id=p.instrument_id,
            volume=vol, price=sell_price, order_direction="sell", offset="open",
        )
        if oid:
            self._my_oids.add(oid)
            self.order_id.add(oid)
    ...
    def on_trade(self, trade: TradeData) -> None:
        ...
        if direction == "sell" and is_open:
            ...
            self._log(
                f"[OPEN SHORT] own_pos {old}→{new} avg={self.avg_price:.2f} "
                f"entry_atr={self._entry_atr:.2f}"
            )
            self._pending = None
            self._save()
        elif direction == "buy" and not is_open:
            # SHORT 平仓: 买平
            entry = self.avg_price
            self._own_pos = max(0, self._own_pos - vol)
            self.state_map.own_pos = self._own_pos
            self.state_map.last_action = f"BUY {vol}手 @ {price}"
            pnl = (entry - price) * vol * self._multiplier   # SHORT pnl
            self._perf.on_close(entry, price, vol, "short")
