"""
================================================================================
  I_Short_Portfolio_V26_V29 — 铁矿石做空组合 (H1+H4 双时间框架)
================================================================================

  组合策略: V26(OI Flow+MACD, H1) + V29(MFI+RSI+EMA, H4)
  信号逻辑:
    H1回调更新 _signal_v26, H4回调更新 _signal_v29 并触发交易
    combined = (_signal_v26 + _signal_v29) / 2 → [-1, 0]
...
STRATEGY_NAME = "I_Short_Portfolio_V26_V29"
...
# ══════════════════════════════════════════════════════════════════════════════
#  SIGNAL — V26 (OI Flow + MACD, H1)
# ══════════════════════════════════════════════════════════════════════════════

def generate_signal_v26(closes, oi, volumes, bar_idx):
    """V26 OI Flow + MACD 做空信号 → [-1, 0]."""
    if bar_idx < V26_WARMUP:
        return 0.0

    flow, _ = _oi_flow(closes, oi, volumes, V26_OI_PERIOD)
    macd_line, macd_sig, _ = _macd(closes, V26_MACD_FAST, V26_MACD_SLOW, V26_MACD_SIGNAL_PERIOD)
...
# ══════════════════════════════════════════════════════════════════════════════
#  SIGNAL — V29 (MFI + RSI + EMA, H4)
# ══════════════════════════════════════════════════════════════════════════════

def generate_signal_v29(closes, highs, lows, volumes, bar_idx):
    """V29 MFI + RSI + EMA 做空信号 → [-1, 0]."""
    if bar_idx < V29_WARMUP:
        return 0.0

    mfi_arr = _mfi(highs, lows, closes, volumes, V29_MFI_PERIOD)
    rsi_arr = _rsi(closes, V29_RSI_PERIOD)
    ema_arr = _ema(closes, V29_EMA_PERIOD)
...
class I_Short_Portfolio_V26_V29(BaseStrategy):
    """铁矿石做空组合 — V26(H1) + V29(H4) 双时间框架"""
    ...
    def __init__(self):
        ...
        # 双时间框架K线生成器
        self.kline_generator_h1 = None
        self.kline_generator_h4 = None
        ...
        self._perf = None
        self._multiplier = 100       # 铁矿石: 100吨/手

    @property
    def main_indicator_data(self):
        return {
            "V26": self.state_map.signal_v26,
            "V29": self.state_map.signal_v29,
            "combined": self.state_map.combined,
    ...
    def on_start(self):
        ...
        # H1 K线 (V26, 不推图表)
        self.kline_generator_h1 = KLineGenerator(
            callback=self.callback_h1,
            real_time_callback=self.real_time_callback_h1,
            exchange=p.exchange,
            instrument_id=p.instrument_id,
            style="H1",
        )
        self.kline_generator_h1.push_history_data()

        # H4 K线 (V29, 驱动交易决策)
        self.kline_generator_h4 = KLineGenerator(
            callback=self.callback_h4,
            real_time_callback=self.real_time_callback,
            exchange=p.exchange,
            instrument_id=p.instrument_id,
            style="H4",
        )
        self.kline_generator_h4.push_history_data()
    ...
    def on_tick(self, tick: TickData):
        # 第一层: K线数据 (永远不能断)
        super().on_tick(tick)
        self.kline_generator_h1.tick_to_kline(tick)
        self.kline_generator_h4.tick_to_kline(tick)
    ...
    def callback_h1(self, kline: KLineData):
        try:
    ...
    def _on_bar_h1(self, kline: KLineData):
        ...
        producer = self.kline_generator_h1.producer
    ...
    def _on_bar_h4(self, kline: KLineData):
        """H4 callback: compute V29, combine with V26, manage positions."""
        signal_price = 0.0
        p = self.params_map
        ...
        # H4 数据准备
        producer_h4 = self.kline_generator_h4.producer
        if len(producer_h4.close) < V29_WARMUP + 2:
            self._push_widget(kline, signal_price)
            return
        ...
        self.output(f"[IND H4] MFI={m_val:.1f} RSI={r_val:.1f} EMA={e_val:.1f} close={close:.1f}")
        ...
        self.output(f"[SIGNAL H4] v26={self._signal_v26:.4f} v29={self._signal_v29:.4f} "
                    f"combined={combined:.4f} forecast={forecast:.1f}")

        # 仓位计算 (Vol Targeting, 四舍五入到整数手)
        ...
        # ── 止损检查 (空头: hard/trail反向, 通过RiskManager + inline) ──
        if net_pos > 0:
    ...
    def real_time_callback_h1(self, kline: KLineData):
        """H1 实时K线 → 不推图表"""
        pass
