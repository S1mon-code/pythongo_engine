...
class I_Short_1H_V26_OI_Flow_MACD(BaseStrategy):
    ...
    def _on_tick_stops(self, tick: TickData):
        ...
        if pos is None:
            return
        raw_pos = pos.net_position   # 做空为负
        price = tick.last_price

        self._risk.update_peak_trough_tick(price, raw_pos)
        self.trough_price = self._risk.trough_price  # 同步给 state_map / save
    ...
    def _on_bar(self, kline: KLineData):
        ...
        if net_pos == 0:
            ...
            # 传原始有符号 net_pos, 本地 net_pos 是 abs, 需取负号
