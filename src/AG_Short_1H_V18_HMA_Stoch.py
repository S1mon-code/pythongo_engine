"""AG Short 1H V18 — HMA Slope + Stochastic (白银1H做空)

QBase_v2 策略: strong_trend_short_AG_1h_v18
信号逻辑:
  HMA(16) slope < 0 AND Stochastic K < 25 → 做空信号
  基础强度 = -0.50
  额外 -0.25 if stoch_k < 15
  额外 -0.15 if slope < -1.0
  Clamp to [-1, 0], 再用 EMA(3) 平滑
  Final signal = min(0.0, smoothed)
仓位: Vol Targeting + Carver 10% buffer
止损: 移动止损(反向) + 2%权益硬止损 + Chandelier Exit(空头) + Portfolio Stops
...
STOCH_K_PERIOD = 6
STOCH_D_PERIOD = 3
SIGNAL_EMA_PERIOD = 3          # 信号平滑 EMA 周期
WARMUP = 20

# Vol Targeting
TARGET_VOL = 0.15
VOL_ATR_PERIOD = 14
ANNUAL_FACTOR = 252 * 10       # 1H bars per year (AG ~10 交易小时/天)
...
def hma(data, period):
    ...
    # Replace NaN with 0 for the final WMA computation
    valid = np.where(np.isnan(diff), 0.0, diff)
    result = wma(valid, sqrt_p)
    # Mask warmup period where data is insufficient
    for i in range(len(result)):
        if i < period + sqrt_p - 2:
            result[i] = np.nan
    return result


def stochastic(highs, lows, closes, k_period=6, d_period=3):
    """Stochastic Oscillator — K 线和 D 线.
    ...
    for i in range(k_period - 1, n):
        ...
        if highest == lowest:
            ...
            k_arr[i] = 100.0 * (closes[i] - lowest) / (highest - lowest)
    # D line = SMA of K
    d_arr = np.full(n, np.nan)
    for i in range(k_period + d_period - 2, n):
        window = k_arr[i - d_period + 1:i + 1]
...
# ═══════════════════════════════════════════════════════════════
# Section 3: SIGNAL (QBase v18 信号逻辑 — SHORT ONLY)
# ═══════════════════════════════════════════════════════════════

def compute_raw_signal(hma_arr, stoch_k_arr, bar_idx):
    """HMA Slope + Stochastic → 空头信号 [-1, 0].

    条件: HMA slope < 0 AND Stochastic K < 25
    基础强度: -0.50
    额外 -0.25 if stoch_k < 15
    额外 -0.15 if slope < -1.0
    Clamp to [-1, 0]
    ...
    hma_curr = hma_arr[bar_idx]
    hma_prev = hma_arr[bar_idx - 1]
    stoch_k = stoch_k_arr[bar_idx]

    if np.isnan(hma_curr) or np.isnan(hma_prev) or np.isnan(stoch_k):
        return 0.0

    slope = hma_curr - hma_prev

    # 入场条件: HMA下降 AND Stochastic超卖区
    if slope >= 0 or stoch_k >= 25:
        return 0.0
...
def compute_smoothed_signal(closes, highs, lows, hma_arr, bar_idx):
    """计算每根bar的raw signal, 用EMA(3)平滑, 返回final signal.

    遍历所有bar计算raw → EMA平滑 → final = min(0.0, smoothed)
    """
    stoch_k_arr, _ = stochastic(highs, lows, closes, STOCH_K_PERIOD, STOCH_D_PERIOD)

    # 收集所有bar的raw signal
    n = len(closes)
    raw_signals = np.zeros(n, dtype=np.float64)
    for i in range(n):
        raw_signals[i] = compute_raw_signal(hma_arr, stoch_k_arr, i)

    # EMA(3) 平滑
    smoothed = ema(raw_signals, SIGNAL_EMA_PERIOD)

    # Final signal: min(0.0, smoothed) — 确保始终 <= 0
    final = min(0.0, smoothed[bar_idx])
    return final, stoch_k_arr
...
def chandelier_exit_short_triggered(lows, closes, atr_arr, bar_idx):
    """Chandelier Exit (空头): close > lowest_low(period) + mult × ATR.
...
STRATEGY_NAME = "AG_Short_1H_V18_HMA_Stoch"
...
class AG_Short_1H_V18_HMA_Stoch(BaseStrategy):
    """白银1H做空 — HMA Slope + Stochastic

    QBase_v2: strong_trend_short_AG_1h_v18
    方向: SHORT ONLY
    指标: HMA(16) slope + Stochastic(6,3) + EMA(3) 信号平滑
    """
    ...
    def on_start(self):
        ...
        self.output(
            ...
            f"H1 SHORT | max={p.max_position} capital={p.capital:,.0f} | "
            f"持仓={actual}"
        )
        feishu(
    ...
    def _on_bar_complete(self, kline: KLineData):
        ...
        # ── 5. 信号计算 (SHORT ONLY) ──
        final_signal, stoch_k_arr = compute_smoothed_signal(
            closes, highs, lows, hma_arr, bar_idx
        )
        # forecast: 负值代表做空强度, 范围 [-FORECAST_CAP, 0]
        forecast = max(-FORECAST_CAP, final_signal * FORECAST_SCALAR)
        ...
        # Chandelier Exit (空头: close > lowest_low + mult × ATR)
        ...
        # 日志: HMA slope + Stoch K
        hma_slope = (
            hma_arr[bar_idx] - hma_arr[bar_idx - 1]
            if bar_idx > 0
            and not np.isnan(hma_arr[bar_idx])
            and not np.isnan(hma_arr[bar_idx - 1])
            else 0.0
        )
        stoch_val = stoch_k_arr[bar_idx] if not np.isnan(stoch_k_arr[bar_idx]) else 0.0
        self.output(
            f"[BAR] HMA_slope={hma_slope:.2f} StochK={stoch_val:.1f} "
