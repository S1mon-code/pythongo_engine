...
class LC_Short_4H_V14_OI_Flow_EMA(BaseStrategy):
    ...
    def _on_bar(self, kline: KLineData):
        ...
        if current == 0:
            self.avg_price = 0.0
            self.trough_price = 0.0
        elif close < self.trough_price or self.trough_price == 0:
        ...
        if current > 0 and self.avg_price > 0:
            ...
            if close >= self.avg_price * (1 + p.hard_stop_pct / 100):
                self._pending = "HARD_STOP"
                self._pending_target = 0
                self._pending_reason = (
                    f"硬止损 close={close:.1f} >= "
                    f"avg*{1+p.hard_stop_pct/100:.4f}="
                    f"{self.avg_price*(1+p.hard_stop_pct/100):.1f}"
                )
                self._push_widget(kline)
                return

            # Trailing stop: close >= trough_price * (1 + trailing_pct/100)
            if self.trough_price > 0 and close >= self.trough_price * (1 + p.trailing_pct / 100):
                self._pending = "TRAIL_STOP"
                self._pending_target = 0
                self._pending_reason = (
                    f"移动止损 close={close:.1f} >= "
                    f"trough*{1+p.trailing_pct/100:.4f}="
                    f"{self.trough_price*(1+p.trailing_pct/100):.1f}"
                )
                self._push_widget(kline)
                return
    ...
    # ══════════════════════════════════════════════════════════════════════
    #  执行 (SHORT: sell to open, buy to close)
    # ══════════════════════════════════════════════════════════════════════
