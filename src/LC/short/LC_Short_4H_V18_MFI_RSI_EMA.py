"""
================================================================================
  LC_Short_4H_V18 — MFI + RSI + EMA 做空 (碳酸锂 H4)
================================================================================

  信号: MFI超买(>70) + RSI>65 + close<EMA(50) → 做空
...
FORECAST_CAP = 20.0
ANNUAL_FACTOR = 252 * 1          # H4: 碳酸锂无夜盘, ~1 bar/day
...
class LC_Short_4H_V18_MFI_RSI_EMA(BaseStrategy):
    """碳酸锂H4做空 — MFI + RSI + EMA"""
    ...
    def on_start(self):
        ...
        super().on_start()
        self.output(f"启动 | {p.instrument_id} {p.kline_style} | 乘数={self._multiplier} | 持仓={actual}")
    ...
    def _on_tick_aux(self, tick: TickData):
        ...
        now = datetime.now()
        if (not self._daily_review_sent and now.hour == DAILY_REVIEW_HOUR
    ...
    def _on_bar(self, kline: KLineData):
        ...
        self.output(f"[IND] MFI={m_val:.1f} RSI={r_val:.1f} EMA50={e_val:.1f} close={close:.1f}")
        ...
        if net_pos == 0:
            ...
            # trough 由 _on_tick_stops 维护, bar close 也参与更新
        ...
        # 止损 (hard/trail 由 _on_tick_stops 处理, bar 级只负责 equity/portfolio/daily)
        if net_pos > 0:
